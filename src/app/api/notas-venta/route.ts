...
export async function GET(request: NextRequest) {
  ...
  const url = query
    ? `${API_URL}/api/notas-venta?${query}`
    : `${API_URL}/api/notas-venta`;
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudieron obtener las notas de venta.") },
...
export async function POST(request: NextRequest) {
  ...
  const backendResponse = await fetch(`${API_URL}/api/notas-venta`, {
    method: "POST",
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudo crear la nota de venta.") },
      { status: backendResponse.status }
    );
  }
