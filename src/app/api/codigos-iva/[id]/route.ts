...
export async function GET(
  ...
  const backendResponse = await fetch(`${API_URL}/api/codigos-iva/${id}`, {
    method: "GET",
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudo obtener el codigo IVA.") },
...
export async function PUT(
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudo actualizar el codigo IVA.") },
      { status: backendResponse.status }
    );
  }
