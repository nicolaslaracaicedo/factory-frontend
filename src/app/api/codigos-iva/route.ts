...
export async function GET(request: NextRequest) {
  ...
  const url = query ? `${API_URL}/api/codigos-iva?${query}` : `${API_URL}/api/codigos-iva`;
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudieron obtener los codigos IVA.") },
...
export async function POST(request: NextRequest) {
  ...
  const backendResponse = await fetch(`${API_URL}/api/codigos-iva`, {
    method: "POST",
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudo crear el codigo IVA.") },
      { status: backendResponse.status }
    );
  }
