...
export async function GET(
  ...
  const backendResponse = await fetch(`${API_URL}/api/productos/${id}`, {
    method: "GET",
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudo obtener el producto.") },
...
export async function PUT(
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudo actualizar el producto.") },
      { status: backendResponse.status }
    );
  }
