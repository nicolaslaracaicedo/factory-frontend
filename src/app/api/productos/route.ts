...
export async function GET(request: NextRequest) {
  ...
  const url = query ? `${API_URL}/api/productos?${query}` : `${API_URL}/api/productos`;
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudieron obtener los productos.") },
...
export async function POST(request: NextRequest) {
  ...
  const backendResponse = await fetch(`${API_URL}/api/productos`, {
    method: "POST",
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudo crear el producto.") },
      { status: backendResponse.status }
    );
  }
