...
export async function GET(request: NextRequest) {
  ...
  const url = query ? `${API_URL}/api/secuenciales?${query}` : `${API_URL}/api/secuenciales`;
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudieron obtener los secuenciales.") },
...
export async function POST(request: NextRequest) {
  ...
  const backendResponse = await fetch(`${API_URL}/api/secuenciales`, {
    method: "POST",
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudo crear el secuencial.") },
      { status: backendResponse.status }
    );
  }
