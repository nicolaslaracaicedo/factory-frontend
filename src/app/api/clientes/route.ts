...
export async function GET(request: NextRequest) {
  ...
  const url = query ? `${API_URL}/api/clientes?${query}` : `${API_URL}/api/clientes`;
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudieron obtener los clientes.") },
...
export async function POST(request: NextRequest) {
  ...
  const backendResponse = await fetch(`${API_URL}/api/clientes`, {
    method: "POST",
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudo crear el cliente.") },
      { status: backendResponse.status }
    );
  }
