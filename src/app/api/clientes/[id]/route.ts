...
export async function GET(
  ...
  const backendResponse = await fetch(`${API_URL}/api/clientes/${id}`, {
    method: "GET",
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudo obtener el cliente.") },
...
export async function PUT(
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudo actualizar el cliente.") },
      { status: backendResponse.status }
    );
  }
