...
export async function GET(request: NextRequest) {
  ...
  const url = query ? `${API_URL}/api/grupos-productos?${query}` : `${API_URL}/api/grupos-productos`;
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudieron obtener los grupos.") },
...
export async function POST(request: NextRequest) {
  ...
  const backendResponse = await fetch(`${API_URL}/api/grupos-productos`, {
    method: "POST",
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudo crear el grupo.") },
      { status: backendResponse.status }
    );
  }
