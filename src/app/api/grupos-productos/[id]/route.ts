...
export async function GET(
  ...
  const backendResponse = await fetch(`${API_URL}/api/grupos-productos/${id}`, {
    method: "GET",
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudo obtener el grupo.") },
...
export async function PUT(
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudo actualizar el grupo.") },
      { status: backendResponse.status }
    );
  }
