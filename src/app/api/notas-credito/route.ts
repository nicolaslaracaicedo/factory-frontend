...
export async function GET(request: NextRequest) {
  ...
  const url = query
    ? `${API_URL}/api/notas-credito?${query}`
    : `${API_URL}/api/notas-credito`;
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudieron obtener las notas.") },
...
export async function POST(request: NextRequest) {
  ...
  const backendResponse = await fetch(`${API_URL}/api/notas-credito`, {
    method: "POST",
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudo crear la nota.") },
      { status: backendResponse.status }
    );
  }
