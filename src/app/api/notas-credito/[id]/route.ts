...
export async function GET(
  ...
  const backendResponse = await fetch(`${API_URL}/api/notas-credito/${id}`, {
    method: "GET",
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudo obtener la nota.") },
...
export async function PUT(
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudo actualizar la nota.") },
      { status: backendResponse.status }
    );
  }
...
export async function DELETE(
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudo eliminar la nota.") },
      { status: backendResponse.status }
    );
  }

  return NextResponse.json(payload ?? { message: "Nota eliminada." });
}
