...
export async function GET(
  ...
  const backendResponse = await fetch(`${API_URL}/api/proveedores/${id}`, {
    method: "GET",
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudo obtener el proveedor.") },
...
export async function PUT(
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudo actualizar el proveedor.") },
      { status: backendResponse.status }
    );
  }
