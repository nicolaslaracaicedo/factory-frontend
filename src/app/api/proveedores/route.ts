...
export async function GET(request: NextRequest) {
  ...
  const url = query ? `${API_URL}/api/proveedores?${query}` : `${API_URL}/api/proveedores`;
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudieron obtener los proveedores.") },
...
export async function POST(request: NextRequest) {
  ...
  const backendResponse = await fetch(`${API_URL}/api/proveedores`, {
    method: "POST",
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudo crear el proveedor.") },
      { status: backendResponse.status }
    );
  }
