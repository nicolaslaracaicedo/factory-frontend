...
export async function GET(request: NextRequest) {
  ...
  const url = query ? `${API_URL}/api/recurrentes?${query}` : `${API_URL}/api/recurrentes`;
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudieron obtener los recurrentes.") },
...
export async function POST(request: NextRequest) {
  ...
  const backendResponse = await fetch(`${API_URL}/api/recurrentes`, {
    method: "POST",
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudo crear el recurrente.") },
      { status: backendResponse.status }
    );
  }
