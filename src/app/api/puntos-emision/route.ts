...
export async function GET(request: NextRequest) {
  ...
  const url = query ? `${API_URL}/api/puntos-emision?${query}` : `${API_URL}/api/puntos-emision`;
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudieron obtener los puntos.") },
...
export async function POST(request: NextRequest) {
  ...
  const backendResponse = await fetch(`${API_URL}/api/puntos-emision`, {
    method: "POST",
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudo crear el punto.") },
      { status: backendResponse.status }
    );
  }
