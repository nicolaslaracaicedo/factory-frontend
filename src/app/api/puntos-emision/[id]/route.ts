...
export async function GET(
  ...
  const backendResponse = await fetch(`${API_URL}/api/puntos-emision/${id}`, {
    method: "GET",
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudo obtener el punto.") },
...
export async function PUT(
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudo actualizar el punto.") },
      { status: backendResponse.status }
    );
  }
