...
export async function GET(request: NextRequest) {
  ...
  const url = query ? `${API_URL}/api/retenciones?${query}` : `${API_URL}/api/retenciones`;
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudieron obtener las retenciones.") },
...
export async function POST(request: NextRequest) {
  ...
  const backendResponse = await fetch(`${API_URL}/api/retenciones`, {
    method: "POST",
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudo crear la retención.") },
      { status: backendResponse.status }
    );
  }
