...
export async function GET(request: NextRequest) {
  ...
  const url = query ? `${API_URL}/api/facturas?${query}` : `${API_URL}/api/facturas`;
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudieron obtener las facturas.") },
...
export async function POST(request: NextRequest) {
  ...
  const backendResponse = await fetch(`${API_URL}/api/facturas`, {
    method: "POST",
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudo crear la factura.") },
      { status: backendResponse.status }
    );
  }
