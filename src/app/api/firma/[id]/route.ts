...
export async function GET(
  ...
  const backendResponse = await fetch(`${API_URL}/api/firma/${id}`, {
    method: "GET",
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudo obtener la firma.") },
...
export async function PUT(
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudo reemplazar la firma.") },
      { status: backendResponse.status }
    );
  }
