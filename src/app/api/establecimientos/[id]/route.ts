...
export async function GET(
  ...
  const backendResponse = await fetch(`${API_URL}/api/establecimientos/${id}`, {
    method: "GET",
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudo obtener el establecimiento.") },
...
export async function PUT(
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudo actualizar el establecimiento.") },
      { status: backendResponse.status }
    );
  }
