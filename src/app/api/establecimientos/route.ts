...
export async function GET(request: NextRequest) {
  ...
  const url = query ? `${API_URL}/api/establecimientos?${query}` : `${API_URL}/api/establecimientos`;
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudieron obtener los establecimientos.") },
...
export async function POST(request: NextRequest) {
  ...
  const backendResponse = await fetch(`${API_URL}/api/establecimientos`, {
    method: "POST",
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudo crear el establecimiento.") },
      { status: backendResponse.status }
    );
  }
