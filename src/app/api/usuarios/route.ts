...
export async function GET(request: NextRequest) {
  ...
  const url = query ? `${API_URL}/api/usuarios?${query}` : `${API_URL}/api/usuarios`;
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudieron obtener los usuarios.") },
...
export async function POST(request: NextRequest) {
  ...
  const backendResponse = await fetch(`${API_URL}/api/usuarios`, {
    method: "POST",
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      { message: toMessage(payload, "No se pudo crear el usuario.") },
      { status: backendResponse.status }
    );
  }
