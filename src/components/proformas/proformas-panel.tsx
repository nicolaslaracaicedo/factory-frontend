...
export function ProformasPanel({ showPanel = true, readOnly = false }: ProformasPanelProps) {
  ...
  const [savedProforma, setSavedProforma] = useState<ProformaItem | null>(null);
  ...
  const totales = form.detalles.reduce<{
    subtotal: number; descuento: number; iva: number; ice: number; irbpnr: number; icePcts: number[]; irbpnrValues: number[]; ivaPcts: number[]; total: number
  }>(
    (acc, d) => {
  ...
  const submitForm = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (editing) {
        ...
        const result = await proformasService.createProforma(payload);
        toast.success("Proforma creada correctamente.");
        setSavedProforma(result);
                      ...
                      {form.detalles.map((detalle, idx) => {
                        ...
                        >
                          ...
                          <span className="text-xs text-amber-600 font-medium">
                            {producto?.tiene_ice ? (iceTotal ?? 0).toLocaleString("es-EC", { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : "-"}
                          </span>
            ...
            >
              <div className="p-6 text-center">
                ...
                <Dialog.Description className="mt-2 text-sm text-slate-500 leading-relaxed">
                  La proforma se ha guardado como borrador. ¿Qué deseas hacer ahora?
                </Dialog.Description>
              </div>
              ...
              <div className="px-6 pb-6 space-y-2">
                <Button
                  ...
                  onClick={() => {
                    if (savedProforma) window.open(`/api/proformas/${savedProforma.id}/pdf`, '_blank');
                  }}
                >
